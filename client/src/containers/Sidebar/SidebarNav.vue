      ...
      <ul class="nav">
        <template v-for="(item, index) in navItems">
          <template v-if="item.title">
            <SidebarNavTitle v-if="!item.hidden" :key="index" :name="item.name" :classes="item.class" :wrapper="item.wrapper"/>
          </template>
          <template v-else-if="item.divider">
            <SidebarNavDivider v-if="!item.hidden" :key="index" :classes="item.class"/>
          </template>
          <template v-else-if="item.label">
            <SidebarNavLabel v-if="!item.hidden" :key="index" :name="item.name" :url="item.url" :icon="item.icon" :label="item.label" :classes="item.class"/>
          </template>
          <template v-else>
            <template v-if="item.children">
              <!-- First level dropdown -->
              <SidebarNavDropdown v-if="!item.hidden" :key="index" :name="item.name" :url="item.url" :icon="item.icon">
                <template v-for="(childL1, index1) in item.children">
                  <template v-if="childL1.children">
                    <!-- Second level dropdown -->
                    <SidebarNavDropdown v-if="!item.hidden" :key="index1" :name="childL1.name" :url="childL1.url" :icon="childL1.icon">
                  ...
                  <template v-else>
                    <SidebarNavItem  v-if="!item.hidden" :key="index1" :classes="item.class">
            ...
            <template v-else>
              <SidebarNavItem v-if="!item.hidden" :key="index" :classes="item.class">
